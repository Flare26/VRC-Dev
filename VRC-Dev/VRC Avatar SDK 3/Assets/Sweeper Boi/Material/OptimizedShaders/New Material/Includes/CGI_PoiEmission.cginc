...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float3 emissionColor0 = 0;
        ...
        if (poiMods.audioLinkTextureExists)
        {
            ...
            {
                emissionStrength0 *= getBandAtTime(_AudioLinkEmissionCenterOutBand, saturate(1 - poiLight.nDotV));
            }
            emissionStrength0 += lerp(_EmissionCenterOutAddAudioLink.x, _EmissionCenterOutAddAudioLink.y, getBandAtTime(_AudioLinkEmissionCenterOutBand, saturate(1 - poiLight.nDotV)));
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(0), float(1), float(1));
    ...
    if (float(1))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,-14.55,11.88,0).xyz) * float(2)) + _Time.x * float(-34)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,-14.55,11.88,0).xyz, float(-34), float(2), float(0), float(0), pos);
        }
    }
    ...
    if (float(0))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(0), float(5), float(2), float(0));
    }
    
    emissionColor0 = hueShift(emissionColor0, frac(float(0) + float(5) * _Time.x) * float(1));
    ...
    float3 emissionColor1 = 0;
        ...
        emissionStrength1 = float(0);
            ...
            if (poiMods.audioLinkTextureExists)
            {
                ...
                {
                    emissionStrength1 *= getBandAtTime(_AudioLinkEmission1CenterOutBand, saturate(1 - poiLight.nDotV));
                }
                emissionStrength1 += lerp(_EmissionCenterOutAddAudioLink1.x, _EmissionCenterOutAddAudioLink1.y, getBandAtTime(_AudioLinkEmission1CenterOutBand, saturate(1 - poiLight.nDotV)));
