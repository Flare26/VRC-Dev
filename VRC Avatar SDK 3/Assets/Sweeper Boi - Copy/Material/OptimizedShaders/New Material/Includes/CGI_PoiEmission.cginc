...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    // First Emission
    float3 emission0 = 0;
    float emissionStrength0 = float(3.51);
    ...
    if (float(1))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,-10,0,0).xyz) * float(20)) + _Time.x * float(50)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,-10,0,0).xyz, float(50), float(20), float(10), float(0), pos);
        }
    }
    ...
    if (float(1))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(1), float(5), float(1), float(0));
    }
    
    emissionColor0 = hueShift(emissionColor0, frac(float(0) + float(10) * _Time.x) * float(1));
