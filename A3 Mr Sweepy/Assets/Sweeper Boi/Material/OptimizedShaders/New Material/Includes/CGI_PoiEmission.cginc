...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    if (float(0))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,10,0,0).xyz) * float(6)) + _Time.x * float(0)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,10,0,0).xyz, float(0), float(6), float(10), float(0), pos);
        }
    }
