...
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float(0.743);
        #else
        ...
        switch(float(1))
        {
            ...
            case 1: // Math Gradient
            {
                ...
                if (float(1))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.743)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.743)));
                }
            }
            break;
            case 2:
        ...
        if (float(1) == 2) // Wrapped
        {
            ...
            uint normalsindex = float(1) > 0 ? 1: 0;
                    ...
                    {
                        poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, float(1)), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.5), 1 - poiLight.lightMap)));
        ...
        float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
        {
            ...
            {
                float uv = saturate(RTWrapFunc(-dotNL, float(0.39), float(0))) * detailShadow;
                poiLight.rampedLightMap = 1;
        ...
        float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
        {
            float3 finalLighting = 1;
                ...
                {
                    float uv = saturate(RTWrapFunc(poiLight.nDotL, float(0.39), float(0))) * detailShadow;
                    poiLight.rampedLightMap = 1;
                ...
                switch(float(1))
                {
                    ...
                    case 3: // Skin
                    {
                        ...
                        finalLighting = min(lerp(indirectLighting * float4(1,1,1,1), float4(1,1,1,1), float(1)) + (sss * directLighting), directLighting);
                    }
                    break;
                    case 4:
                    {
