...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    #endif
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(0.422), float(1), float(1), float(1));
    #if defined(PROP_EMISSIONMAP) || !defined(OPTIMIZER_ENABLED)
    ...
    if (float(0))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,10,0,0).xyz) * float(16)) + _Time.x * float(50)).r;
            #endif
        }
        else
        {
            emissionStrength0 *= calculateScrollingEmission(float4(0,10,0,0).xyz, float(50), float(16), float(10), float(0), pos);
        }
    }
