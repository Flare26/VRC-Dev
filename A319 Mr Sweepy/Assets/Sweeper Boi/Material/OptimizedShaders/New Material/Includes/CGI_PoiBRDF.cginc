...
float3 getDirectSpecular(float roughness, float ndh, float vdn, float ndl, float ldh, float3 f0, float3 halfVector, float3 tangent, float3 bitangent, float anisotropy)
{
        ...
        return lerp(directSpecularNonAniso, directSpecularAniso, saturate(abs(float(-0.3) * 100))) * 3; // * 100 to prevent blending, blend because otherwise tangents are fucked on lightmapped object
    #else
        return 0;
    #endif
}
...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * float(0.572);
    float reflectance = metallicGlossMap.g * float(0.5);
    float roughness = max(1 - (float(0.803) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
    finalColor.rgb *= lerp(1, 1 - metallic, float(0));
    float3 reflViewDir = getAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[1], roughness, float(-0.3));
    ...
    float3 directSpecular = getDirectSpecular(roughness, saturate(poiLight.nDotH), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.halfDir, poiMesh.tangent.xyz, poiMesh.binormal, float(-0.3)) * poiLight.attenuation * attenuation * poiLight.color;
    directSpecular = min(directSpecular, poiLight.color);
    float3 vDirectSpecular = 0;
    #ifdef VERTEXLIGHT_ON
        for (int index = 0; index < 4; index++)
        {
            float3 v0directSpecular = getDirectSpecular(roughness, saturate(poiLight.vDotNH[index]), max(poiLight.nDotV, 0.000001), attenuation, saturate(poiLight.lDotH), f0, poiLight.vHalfDir[index], poiMesh.tangent, poiMesh.binormal, float(-0.3)) * poiLight.attenuation * poiLight.vAttenuationDotNL[index] * poiLight.vColor[index];
    ...
    float3 specular = indirectSpecular * float(0) * metallicTintMask.a * metallicTintMask.rgb * poiLight.occlusion + (directSpecular + vDirectSpecular) * float(1) * spcularTintMask.a * spcularTintMask.rgb;
