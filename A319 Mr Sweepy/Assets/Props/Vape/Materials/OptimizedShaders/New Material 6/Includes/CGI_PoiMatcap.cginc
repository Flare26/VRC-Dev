    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(0,1,0.9806237,1);
        #else
            matcap = float4(0,1,0.9806237,1);
        #endif
        ...
        if(float(1))
        {
            matcap.rgb = hueShift(matcap.rgb, float(0.234) + _Time.x * float(0));
        }
