    ...
    float3 RandomColorFromPoint(float2 rando)
    {
        ...
        fixed value = lerp(float4(0.2923849,0.9999999,0,1).x, float4(0.2923849,0.9999999,0,1).y, rando.y);
    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        if(float(1) == 1 || float(0) != 0 && !float(0))
        {
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(40), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(40) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(40) * glitterTexture.a * glitterMask;
        }
    }
#endif
