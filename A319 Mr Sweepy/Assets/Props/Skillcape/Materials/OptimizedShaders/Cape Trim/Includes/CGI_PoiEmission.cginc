...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float3 emissionColor0 = 0;
        ...
        if (poiMods.audioLinkTextureExists)
        {
            ...
            if (float(1))
            {
                emissionStrength0 *= poiMods.audioLink[float(2)];
            }
            
            if (float(0))
            {
                emissionStrength0 *= getBandAtTime(float(2), saturate(1 - poiLight.nDotV), float(1));
            }
            emissionStrength0 += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, getBandAtTime(float(2), saturate(1 - poiLight.nDotV), float(1)));
            emissionStrength0 += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[_AudioLinkAddEmissionBand]);
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(1,1,0.01938236,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(1,1,0.01938236,1).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, float(0)).rgb * float4(1,1,0.01938236,1).rgb;
    #endif
