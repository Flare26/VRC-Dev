...
void calculateBasePassLightMaps()
{
        ...
        switch(float(0))
        {
            ...
            case 1: // Math Gradient
            {
                ...
                if (float(0))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.2)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.2)));
                }
            }
            break;
            case 2:
